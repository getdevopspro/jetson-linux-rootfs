...
target "build" {
  ...
  args = {
    ...
    JETSON_LINUX_RELEASE_URI         = split(".", jetson_version.number)[0] == "35" ? "https://developer.nvidia.com/downloads/embedded/l4t/r35_release_v${split(".", jetson_version.number)[1]}.${split(".", jetson_version.number)[2]}" : null
    JETSON_LINUX_ROOTFS_PACKAGE_NAME = split(".", jetson_version.number)[0] == "35" ? "tegra_linux_sample-root-filesystem_r35.${split(".", jetson_version.number)[1]}.${split(".", jetson_version.number)[2]}_aarch64.tbz2" : null,
    JETSON_LINUX_ROOTFS_PACKAGE_URL  = split(".", jetson_version.number)[0] == "35" ? "https://developer.nvidia.com/downloads/embedded/l4t/r35_release_v${split(".", jetson_version.number)[1]}.${split(".", jetson_version.number)[2]}/tegra_linux_sample-root-filesystem_r35.${split(".", jetson_version.number)[1]}.${split(".", jetson_version.number)[2]}_aarch64.tbz2" : null,
  }
  tags = concat(
...
target "build-variants" {
  ...
  name       = "${replace(sanitize(IMAGE_NAME), "_", "-")}-${replace(sanitize(jetson_version.number), "_", "-")}-${replace(sanitize(samplefs_variant), "_", "-")}"
  platforms = [
    "linux/arm64",
  ]
  matrix = {
  ...
  args = {
    ...
    JETSON_SAMPLEFS_VERSION = jetson_version.ubuntu_release,
  }
  tags = concat(
    jetson_version.number == JETSON_VERSIONS[0].number ? ["${IMAGE_NAME}-${samplefs_variant}:latest"] : [],
    [
      "${IMAGE_NAME}-${samplefs_variant}:${split(".", jetson_version.number)[0]}",
      "${IMAGE_NAME}-${samplefs_variant}:${split(".", jetson_version.number)[0]}.${split(".", jetson_version.number)[1]}",
      "${IMAGE_NAME}-${samplefs_variant}:${jetson_version.number}",
      "${IMAGE_NAME}-${samplefs_variant}:${jetson_version.number}-${formatdate("YYYYMMDDhhmm", timestamp())}",
    ]
  )
  labels = {
    "manifest:org.opencontainers.image.version" : jetson_version.number
